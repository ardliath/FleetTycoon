...
**Neglect must bite.** *(First pass built — Phase 6 chunk 1, 2026-07-21; see docs/ROADMAP.md for the mechanism and numbers.)* Left running unattended, the game used to just accrue money — no reason to attend, which hollowed out the loop. Now every owned ship suffers passive condition decay (net drift even with automatic routine upkeep), and a run-down ship increasingly *fails to sail at all* (`sim/captain.ts`'s hazard-independent breakdown channel → cancellations → cratered reliability → lost contracts): "broken ships and nothing sailing," on every route rather than only exposed ones. The player's deliberate `applyMaintenance` spend is the cheap counter-lever, so the cost of neglect is paid in *presence*, not cash. Paired with a real global pause (the humane counterpart — step away deliberately at no cost). Still to come on this axis: **fast-forward** through quiet stretches, the idle **digest**, and a decision on whether repaired ships should permanently age (drydock currently resets them to pristine). This was the single sharpest gap between "idle money-printer" and "management game," and it's what gives the stewardship trigger something to be anxious *about*.
...
- **Pause & time controls**: a real-time game with 10–15-minute days needs an answer for stepping away — pause, and possibly fast-forward through quiet stretches. **Pause resolved (Phase 6, 2026-07-21):** a real global pause exists (topbar toggle; `GameContext`'s day-clock effect genuinely halts — no ticks, no decay — while paused), the humane counterpart to neglect-decay. Two related pieces stand unchanged/open: the ~~in-notice behaviour~~ — **Phase 2, 2026-07-18:** a docking notice left unattended still auto-resolves via the captain's risk roll, no hard pause forced; and **fast-forward** through quiet stretches is still undiscussed (deliberately deferred as its own later increment).
